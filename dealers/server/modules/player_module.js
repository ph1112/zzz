...
class player_module extends base_module
{
	...
	service(){
		...
		this.app.get('/set_dealer_account_token',function (req,res,next){
            console.log("call set_dealer_account_token");
				this.set_dealer_account_token(req,res,next);
			}.bind(this) );
    };

 	set_dealer_account_token(req,res,next){
        ...
        var ret = this.setAccountToken(account,token,gems);
        console.log("ret:" + ret);
        if(!ret){
            this.send(res,1,"set token failed");
            return;
        }
        else{
            this.send(res,0,"ok",{
	...
	transfer_gems2user(req,res,next){
        console.log("chongzhiqule....");
		var token = req.query.token; //充值人的账户token
        var userid = req.query.userid; //被充值人的ID
        ...
        if(dealer.gems < num){
            ...
            if(ret){
                ...
                if( this.addUserGems(dealer.account,userdata.userid,num) )
                {
...
//=====================================================================
    ...
    /**增加玩家房卡 */
    addUserGems(account,userid,gems){
        var back = null;
        var f = Fibers.current;
        this.http.get(this.user_host,this.user_port,this.user_add_path,{dealer:account,userid:userid,gems:gems},function(state,result){
            //console.log(state);
            console.log(result);
