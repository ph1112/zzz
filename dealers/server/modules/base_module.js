...
class base_module
{
	...
	/**设置数据库token */
	setAccountToken(account,token,gems){
		var ret = this.getDealerByAccount(account);
		var sql = '';
		if(ret == null){
			var psw = Date.now();
			var time = Date.now();
			sql = 'INSERT INTO t_dealers(account,token,password,name,create_time,gems) VALUES("{0}","{1}","{2}","{3}",{4},{5})';
			sql = sql.format(account,token,psw,account,time,gems);
		}
		else{
			sql = 'UPDATE t_dealers SET token="{0}",gems={1} WHERE account="{2}"';
			sql = sql.format(token,gems,account);
			//sql = 'UPDATE t_dealers SET token ="' + token + '"WHERE account = "' + account + '"'; 
		}			
		var ret = this.query(sql);		
		console.log("retttt:" + JSON.stringify(ret));
		if(ret.err){
